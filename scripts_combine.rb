...
class Numeric
  def to_digits(num = 3)
    str = to_s
    (num - str.size).times { str.prepend("0") }
    return str
  end
end
...
module Scripts
  ...
  def self.createLoaderScripts(rxdata)
    ...
    File.open(rxdata, "wb") do |f|
      Marshal.dump([[62_054_200, "Main", txt]], f)
    end
  end
  ...
  def self.aggregate_from_folder(path, scripts, level = 0)
    ...
    files.each do |f|
      ...
      content = File.binread(path + "/" + f) # .gsub(/\n/, "\r\n")
  ...
  def self.create_directory(path)
    ...
    paths.each_with_index do |_e, i|
      Dir.mkdir(paths[0..i].join("/")) if !File.directory?(paths[0..i].join("/"))
    end
  end
end
