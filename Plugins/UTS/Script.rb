...
#===============================================================================
# Enhanced Pokemon UI Legacy Data compatibility
#===============================================================================
if UnrealTime::ENABLED && defined?(Pokemon)
  class Player < Trainer
    alias :uts_update_party_time :update_party_time if method_defined?(:update_party_time)
    alias :uts_last_update_time :last_update_time if method_defined?(:last_update_time)
    
    def update_party_time
      @party.each { |p| p.update_party_time } if @party
    end
    
    def last_update_time(time = nil)
      time = $PokemonGlobal.newSecondCount if !time
      @party.each { |p| p.last_update_time = time } if @party
    end
  end
  ...
  class Pokemon
    ...
    def update_party_time
      now = $PokemonGlobal.newSecondCount
      legacy_data[:party_time] += (now - self.last_update_time) if defined?(@legacy_data)
    end
  end
end