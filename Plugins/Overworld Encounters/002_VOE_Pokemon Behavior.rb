def pbInteractOverworldEncounter
  ...
  textcol = (pkmn.genderless?) ? "" : (pkmn.male?) ? "" : ""
...
def pbTrainersSeePkmn(evt)
  ...
  $game_map.events.each_value do |event|
    ...
    distance = $~[1].to_i
    #echoln("event: #{event.name}")
    #echoln("evt: #{evt}, #{evt.x}, #{evt.y}")
    #echoln("distance: #{distance}")
...
def pbPokemonIdle(evt)
  ...
  if rand(225)==1 || (!VoltseonsOverworldEncounters::GRASS_TILES.include?($game_map.terrain_tag(evt.x, evt.y).id) && !$PokemonEncounters.has_cave_encounters? && !$PokemonGlobal.diving) || ($game_map.terrain_tag(evt.x, evt.y).id != :UnderwaterGrass && $PokemonGlobal.diving)
